...
async def chat_bg(input,input_message, kickoff_id,create_date, API_NAME):
    ...
    """
    USING LiteLLM Python SDK  for unified API in OpenAPI format with llm_config.yaml containing all models
    """

    def load_config(config_path: str = "llm_config.yaml") -> dict:
        with open(config_path, "r") as f:
            config = yaml.safe_load(f)
        # Optionally, replace environment variables in your config here.
        for provider, details in config.items():
            if isinstance(details.get("api_key"), str) and details["api_key"].startswith("${"):
                env_var = details["api_key"].strip("${}")
                details["api_key"] = os.environ.get(env_var)
        return config

    def get_api_config(model_name: str, config: dict) -> dict:
        """
        Returns the API configuration for the given model name.
        """
        for provider, details in config.items():
    ...
    # Usage:
    config = load_config()
    model_name = API_NAME  # your chosen model
    api_config = get_api_config(model_name, config)

    # Now you can use these parameters when initializing your API call:
    api_key_choosen_llm = api_config["api_key"]
    base_url = api_config["base_url"]
    extra_headers = api_config["extra_headers"]
    ...
    api_response= completion(
        ...
        metadata={
            "trace_user_id": entity_id,
            "session_id": kickoff_id,
